...
**Prefiks tagu dla tego repozytorium:** `nfz-umwapi/v` (np. `nfz-umwapi/v1.2.3`)
...
2. Utwórz i wypchnij tag:
   ...
   git tag nfz-umwapi/v1.2.3
   git push origin nfz-umwapi/v1.2.3
   ```

3. Workflow `release.yml` automatycznie:
   - zbuduje projekt,
   - opublikuje paczkę `TBJ.Integrations.NFZ.UmwApi` na NuGet.org (Trusted Publishing) i GitHub Packages,
   - utworzy GitHub Release,
   - zamrozi kod w gałęzi `release/nfz-umwapi/1.2.3`.
...
1. Utwórz gałąź z właściwej gałęzi release:
   ...
   ```bash
   git checkout -b hotfix/nfz-umwapi-1.2.3 release/nfz-umwapi/1.2.3
   ```

   Możesz też utworzyć ją z `main`, jeśli poprawka jest już tam wprowadzona.
2. Wprowadź poprawki.
3. Otwórz PR do gałęzi `release/nfz-umwapi/1.2.3`.
4. Po zatwierdzeniu i merge wypchnij nowy tag:

   ```bash
   git tag nfz-umwapi/v1.2.4
   git push origin nfz-umwapi/v1.2.4
   ```
...
Publikacja na NuGet.org odbywa się przez Trusted Publishing (OIDC). Na NuGet.org dla paczki `TBJ.Integrations.NFZ.UmwApi` musi być skonfigurowana polityka zaufania dla repozytorium `tbudaj/TBJ.Integrations.NFZ.UmwApi` i workflow `release.yml`.
